...
async function loadShoppingCloud() {
  try {
    const userId = getCurrentCloudUser();
    const data = await supabaseSelect("shopping_list", userId);
    return ensureArray(data);
  } catch (error) {
    console.error("Cloud shopping betöltési hiba:", error);
    return [];
  }
}
...
async function saveShoppingCloud(list) {
  try {
    const userId = getCurrentCloudUser();
    const safeList = ensureArray(list).map((item) => ({
      ...item,
      id: ensureItemId(item),
      user_id: userId,
      updated_at: new Date().toISOString(),
    ...
    await supabaseDeleteAllForUser("shopping_list", userId);
    await supabaseInsertMany("shopping_list", safeList);

    return true;
  } catch (error) {
    console.error("Cloud shopping mentési hiba:", error);
    return false;
  }
}
